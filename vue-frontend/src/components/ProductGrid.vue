<template>
        <div class="product-grid">
          <div v-for="product in products" :key="product.id" class="product-card">
            ...
            <div>
              <i v-for="index in product.star" :key="index" class="fas fa-star" style="color: rgb(232, 232, 117);"></i>
            </div>
            <p class="product-price">{{ product.price }} Lei</p>
            
            <button class="custom-button" @click="addToShoppingCart(product.id)"><i class="fas fa-shopping-cart"></i>Adauga in cos</button>
          </div>
        </div>
</template>
