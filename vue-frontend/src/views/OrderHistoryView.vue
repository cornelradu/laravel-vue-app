  ...
  <div class="container">
    <div class="order" v-for="(value, key) in orders" :key="key">
      ...
      <div v-for="item in value" :key="item.product_id" class="orderItem">
        ...
        <div>
          # {{ item.count }} Total: {{ item.count * item.product.price }} Lei
        </div>
      </div>
    ...
    axios
      ...
      .then((response) => {
        ...
        for (let orderItem of response.data) {
          axios
            .get(`${this.apiUrl}/api/products/` + orderItem.product_id, {})
            .then((response) => {
              let image = "images/public/" + response.data.image;

              if (!(orderItem.order_id in this.orders)) {
                this.orders[orderItem.order_id] = [
                  {
                ...
                this.orders_total[orderItem.order_id] =
                  orderItem.count * response.data.price;
                this.orders_timestamp[orderItem.order_id] = orderItem.timestamp;
              } else {
                this.orders[orderItem.order_id].push({
                  product_id: orderItem.product_id,
                  count: orderItem.count,
                  image: image,
                  product: response.data,
